...
// import AuthModal from '@/components/auth-modal'; // Rimane semplificato e non usato attivamente
// import { useToast } from '@/hooks/use-toast';
// import { useAuth } from '../../contexts/auth-context'; // Usiamo il percorso relativo per ora
...
import { Button } from '@/components/ui/button'; // Manteniamo Button per l'header

// BrandLogoIcon semplificato
const BrandLogoIcon = ({ className }: { className?: string }) => (
  <div className={cn("p-2 bg-primary text-primary-foreground text-xs", className)}>Logo Semplice</div>
);
...
export default function AppShell({ children }: AppShellProps) {
  ...
  // const { user, loading: authLoading, logout } = useAuth(); // Logica auth temporaneamente rimossa
  ...
  if (!mounted /* || authLoading */) { // authLoading è commentato
    return (
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm">
            ...
            <p className="text-lg text-muted-foreground">Caricamento AppShell (super-semplificata)...</p>
        </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-40 flex h-16 items-center gap-x-4 border-b bg-background/80 px-4 backdrop-blur-sm sm:px-6 print:hidden">
        ...
        {/* {user ? (
          <Button onClick={() => alert("Logout Placeholder")}>Logout</Button>
        ) : ( */}
